...
Option B) open OrcaSlicer, go to print-settings-tab->output-options. Locate the window for post-processing-script. 
...
=>OrcaSlicer will execute the script after the export of the Gcode, therefore the view in the window wont change. Open the finished gcode file to see the results.
If you want to change generation settings: Scroll to 'Parameter' section. Settings from OrcaSlicer will be extracted automaticly from the gcode.
Requirements:
Python 3.5+ and the librarys: shapely 1.8+, numpy 1.2+, numpy-hilbert-curve matplotlib for debugging
Slicing in OrcaSlicer is mandatory.
Tested only in OrcaSlicer 2.5&Python 3.10, other versions might need adapted keywords.
Notes:
...
#from HilbertCurve import HilbertCurve, HilbertCurve2D, HilbertCurve3D
...
def makeFullSettingDict(gCodeSettingDict:dict) -> dict: 
    ...
    AddManualSettingsDict={
        ...
        "ArcCenterOffset":1.2, # Unit:mm, prevents very small Arcs by hiding the center in not printed section. Make 0 to get into tricky spots with smaller arcs.
        ...
        "TravelSpeed":gCodeSettingDict.get("travel_speed", 6)*60, #Unit:mm/min
        "ExtendIntoPerimeter":1.5*gCodeSettingDict.get("perimeter_extrusion_width", 0.4), #min=0.5extrusionwidth!, extends the Area for arc generation, put higher to go through small passages. Unit:mm
        "MaxDistanceFromPerimeter":2*gCodeSettingDict.get("perimeter_extrusion_width", 0.4),#Control how much bumpiness you allow between arcs and perimeter. lower will follow perimeter better, but create a lot of very small arcs. Should be more that 1 Arcwidth! Unit:mm
        "MinArea":0.0, #Unit:mm2
        "MinBridgeLength":0.0, #Unit:mm
        "Path2Output":r"", #leave empty to overwrite the file or write to a new file. Full path required.
        "RMax":120, # the max radius of the arcs.
        ...
        "ArcWidth":gCodeSettingDict.get("nozzle_diameter",gCodeSettingDict.get("Fallback_nozzle_diameter",0.4))*0.95, #change the spacing between the arcs,should be nozzle_diameter
        "NozzleDiameter":gCodeSettingDict.get("nozzle_diameter", gCodeSettingDict.get("Fallback_nozzle_diameter",0.4)), # use fallback as default
...
def main(gCodeFileStream,path2GCode,skipInput)->None:
    ...
    if not checkforNecesarrySettings(gCodeSettingDict):
        warnings.warn("Incompatible OrcaSlicer-Settings used!")
            ...
            if idl<1:
                ...
                if modify:
                    ...
                    for idline,line in enumerate(layer.lines):
                        if layer.validpolys:
                            # inject arcs at the very start
                            if ";TYPE" in line.replace(" ", "") and not isInjected:
                                ...
                                for id in reversed(range(injectionStart)):
                                    if "X" in layer.lines[id]:
                                        ...
                                        break
                        if layer.oldpolys:
                            # startpoint of solid infill: print all hilberts from here.
                            if ";TYPE" in line.replace(" ", "") and not hilbertIsInjected:
...
def prompt_retry():
    ...
    retry = messagebox.askretrycancel(
        "Retry", "Would you like to select another file?")
    root.destroy()
    return retry
...
def splitGCodeIntoLayers(gcode:list)->list:
    ...
    for linenumber,line in enumerate(gcode):
        if ";CHANGE_LAYER" in line.replace(" ", "") or ";LAYER_CHANGE" in line.replace(" ", ""):
...
class Layer():
    ...
    def extract_features(self)->None:
        ...
        for idl,line in enumerate(self.lines):
            if ";TYPE:" in line.replace(" ", "") or ";FEATURE:" in line.replace(" ", ""):
                if currenttype:
                    self.features.append([currenttype,buff,start])
                    buff=[]
                    start=idl
                currenttype=line.split(":")[-1].replace("\n","").removeprefix(" ").replace(" ","").replace("\t","")
            else:
                buff.append(line)
    ...
    def addHeight(self):
        for l in self.lines:
            if ";Z_HEIGHT" in l.replace(" ", "") or ";HEIGHT" in l.replace(" ", ""):
                h=l.split(":")
                self.height=float(h[-1].replace("\n",""))
                return self.height
    ...
    def spotFeaturePoints(self,featureName:str,splitAtWipe=False,includeRealStartPt=False, splitAtTravel=False)->list:
        ...
        for idf,fe in enumerate(self.features):
            ...
            pts=[]
            isWipeMove=False
            travelstr=f"F{self.parameters.get('travel_speed')}"
    ...
    for line in gcodeLines:
        if "; CONFIG_BLOCK_START" in line:
            ...
            if "; CONFIG_BLOCK_END" in line:
                ...
                continue
        if isSetting :
            setting=line.replace("; ","").strip("\n").split("=")
            if len(setting)==2 and len(setting[1])>0:
                try:
                    gCodeSettingDict[setting[0]]=literal_eval(setting[1].strip(" ").strip("%")) # automaticly convert into int,float,...
                except:
                ...
                #warnings.warn(f"OrcaSlicer Setting {setting[0]} not in the expected key/value format, but added into the settings-dictionarry")
            else:    
                print("Could not read setting from OrcaSlicer:",setting)
    if "%" in str(gCodeSettingDict.get("perimeter_extrusion_width", 0.4)) : #overwrite Percentage width as suggested by 5axes via github                
        gCodeSettingDict["perimeter_extrusion_width"]=gCodeSettingDict.get("nozzle_diameter")*(float(gCodeSettingDict.get("perimeter_extrusion_width", 0.4).strip("%"))/100)
    isWarned=False    
...
def checkforNecesarrySettings(gCodeSettingDict:dict)->bool:
    if gCodeSettingDict.get("use_relative_e_distances") == 0:
        warnings.warn("Script only works with relative e-distances enabled in OrcaSlicer. Change acordingly.")
        return False
    if gCodeSettingDict.get("line_width",0.4)<0.001 or gCodeSettingDict.get("perimeter_extrusion_width", 0.4)<0.001 or gCodeSettingDict.get("internal_solid_infill_line_width",0.4)<0.001:
        warnings.warn("Script only works with extrusion_width and perimeter_extrusion_width and solid_infill_extrusion_width>0. Change in OrcaSlicer acordingly.")
        return False    
    if gCodeSettingDict.get("detect_overhang_wall") == 0:
        warnings.warn("Overhang detection disabled in OrcaSlicer. Activate in OrcaSlicer for script success!")
        return False
    if gCodeSettingDict.get("bridge_speed",5)>5:
        warnings.warn(f"Your Bridging Speed is set to {gCodeSettingDict.get('bridge_speed'):.0f} mm/s in OrcaSlicer. This can cause problems with warping.<=5mm/s is recommended")        
    if gCodeSettingDict.get("infill_first"):
        warnings.warn("Infill set in OrcaSlicer to be printed before perimeter. This can cause problems with the script.")
        return False
    if gCodeSettingDict.get("external_perimeters_first"):
        warnings.warn("OrcaSlicer-Setting: External perimeter is printed before inner perimeters. Change for better overhang performance. ")
    if not gCodeSettingDict.get("avoid_crossing_perimeters"):
        warnings.warn("OrcaSlicer-Setting: Travel Moves may cross the outline and therefore cause artefacts in arc generation.")    
    return True
